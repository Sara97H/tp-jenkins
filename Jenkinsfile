    ...
    post {
        failure {
            script {
                ...
                if (isUnix()) {
                    sh "python3 send_email.py failure \"${env.BUILD_URL}\" \"${env.JOB_NAME}\""
                } else {
                    bat "python send_email.py failure \"${env.BUILD_URL}\" \"${env.JOB_NAME}\""
                }
            }
        }
        fixed {
            script {
                ...
                if (isUnix()) {
                    sh "python3 send_email.py fixed \"${env.BUILD_URL}\" \"${env.JOB_NAME}\""
                } else {
                    bat "python send_email.py fixed \"${env.BUILD_URL}\" \"${env.JOB_NAME}\""
                }
            }
        }
        success {
